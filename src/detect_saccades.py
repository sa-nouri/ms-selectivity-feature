...
def detect_saccades(
    ...
    for i in range(1, len(velocities) - 1):
        ...
        if velocity_magnitude >= velocity_threshold:
            ...
            if current_saccade['duration'] >= params['min_duration']:
                current_saccade['amplitude'] = compute_amplitude(x_positions, y_positions, saccade_start, i-1)
    ...
    if current_saccade is not None:
        current_saccade['duration'] = current_saccade['end_time'] - current_saccade['start_time']
        if current_saccade['duration'] >= params['min_duration']:
            current_saccade['amplitude'] = compute_amplitude(x_positions, y_positions, saccade_start, len(x_positions)-1)
            saccades.append(current_saccade)
...
def validate_saccades(saccades, min_duration=0, min_amplitude=0, max_amplitude=None):
    ...
    for s in saccades:
        ...
        if s.get('duration', 0) >= min_duration and s.get('amplitude', 0) >= min_amplitude:
            if max_amplitude is None or s.get('amplitude', 0) < max_amplitude:
                validated.append(s)
    return validated


def detect_saccades(x_positions, y_positions, timestamps, min_duration=50, threshold_multiplier=6.0):
